/**
 * VaultCrypto — Zero-Knowledge Encryption Module
 *
 * All encryption happens client-side. IPFS only ever sees encrypted blobs.
 *
 * Key derivation: PBKDF2-SHA512, 600,000 iterations
 * Encryption: AES-256-GCM with random 12-byte IV
 * Master key: Never stored, derived from master password each session
 */
...
const VaultCrypto = (() => {
    ...
    /**
     * Generate a random salt (Uint8Array 32 bytes)
     */
     ...
     */
    async function encrypt(masterPassword, salt, plaintext) {
     ...
     */
    async function decrypt(masterPassword, salt, encryptedObj) {
        try {
            if (!encryptedObj || !encryptedObj.iv || !encryptedObj.data) return null;

            const ivBuf = base64ToUint8Array(encryptedObj.iv);
            const dataBuf = base64ToUint8Array(encryptedObj.data);
    ...
    /**
     * Hash master password for verification (stored locally, not on IPFS)
     * Uses PBKDF2 with fewer iterations (fast verification)
     */
    async function hashPassword(password, salt) {
        const derived = await derive(password, salt, 100000);
    ...
    /**
     * Utility: ArrayBuffer/Uint8Array to Base64
     */
    ...
    /**
     * Generate a random recovery phrase (12 words from BIP39 wordlist)
     */
    ...
    const WORDLIST = [
        ...
        'bridge','bright','bring','brisk','broccoli','broken','bronze','broom','brother','brown',
        ...
        'impose','improve','impulse','include','income','increase','index','indicate','indoor','industry',
        ...
        'style','subject','submit','subway','success','such','sudden','sugar','suggest','suit',
        ...
        'sweet','swim','swing','switch','symbol','symptom','syrup','system','table','tackle',
